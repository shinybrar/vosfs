...
| --- | --- |
| fsspec | Run the reusable abstract `open`, `pipe`, `copy`, `get`, and `put` suites plus dedicated `mv` tests through their supported seams. The current hermetic baseline collects 548 tests: 542 passed and six skipped. The abstract subset is 131 passed and the same six `fil?1` copy/get/put skips because the existing path grammar reserves `?` as a query delimiter. |

...
| [Same-source directory `mv` rejection](fsspec-cli-same-source-mv-command-profile.md#directory-admission-boundary) | source | `async source / source-reported directory` | `unsupported` | — | Hermetic negative rejection | [`e8a3d4a`](https://github.com/shinybrar/vosfs/commit/e8a3d4ac72ffa1baffdad98692db250bf7a5f403) `test_mv.py`; `_info` required, no target resolution, staging, or `_mv` |

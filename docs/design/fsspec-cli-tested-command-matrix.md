...
| [Capability-enabled guarded recursive `rm -R`/`-r`](fsspec-cli-rm-recursive-command-profile.md) | source | `local / adapted async` | `unverified` | — | Host qualification plus symlink-race evidence; no positive Local claim | — |
| [Capability-enabled guarded recursive `rm -R`/`-r`](fsspec-cli-rm-recursive-command-profile.md) | source | `memory / adapted async` | `unverified` | — | Hermetic `test_recursive_rm.py` and `test_command_matrix.py` intended-positive qualification plus isolated wheel | — |
| [Capability-enabled guarded recursive `rm -R`/`-r`](fsspec-cli-rm-recursive-command-profile.md) | source | `vosfs / native async` | `unverified` | — | Fully mocked `test_vosfs_command_matrix.py` intended-positive qualification plus isolated wheel | — |
| [Capability-enabled guarded recursive `rm` preflight](fsspec-cli-rm-recursive-command-profile.md#2-capability-and-source-free-preflight) | command preflight | `not entered` | `unverified` | — | Hermetic `test_recursive_rm.py` negative rejection plus isolated wheel | — |
| [Application-disabled recursive `rm -R`/`-r`](fsspec-cli-rm-recursive-command-profile.md#2-capability-and-source-free-preflight) | command preflight | `not entered` | `unverified` | — | Hermetic `test_rm.py` and `test_recursive_rm.py` negative rejection plus isolated wheel | — |

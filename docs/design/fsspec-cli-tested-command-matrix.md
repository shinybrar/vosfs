...
| [Same-source `cp -R` rejection](fsspec-cli-recursive-cp-rejection-profile.md) | command preflight | `not entered` | `unsupported` | `unsupported` | Hermetic negative rejection | Hermetic `test_command_matrix.py::test_cp_option_rejection_is_source_free` on this change |

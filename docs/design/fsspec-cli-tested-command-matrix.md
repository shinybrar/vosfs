...
| [Capability-enabled guarded recursive `rm -R`/`-r`](fsspec-cli-rm-recursive-command-profile.md) | source | `vosfs / native async` | `unverified` | — | Fully mocked `test_vosfs_recursive_command_matrix.py` intended-positive qualification plus isolated wheel | — |
